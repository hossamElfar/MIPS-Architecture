...
public class Datapath {
	...
	public static void decode() throws Exception{
		...
		DX.setReadValue1(RF.ReadReg1(inst.substring(6,11)));
		DX.setReadValue2(RF.ReadReg2(inst.substring(11,16)));
	...
	public static void execute() throws Exception{
		...
		//Jump part
		String jumpaddress = FD.getInstruction().substring(6, 32);
		String jumpPc = FD.getIncrementedPC().substring(0, 5);
		String shifted = jumpaddress + "00";
		M5 = new MUX();
		M5.setInputs(pc,(jumpPc + shifted));
		if(C.jump)c = 1;
		else c = 0;
		pc = M5.select(c);
		c = 0;
	...
	public static void memory(){
		...
		String DMResult = DM.memory(EM.ALUresult, EM.ValueToMem);
		if(EM.isBranch() == true && A.isZero() == true)PcSrc = true;
		MW.setAluResult(EM.ALUresult);
	...
	public static void printDatamemory(){
		for(int i = 0; i < 32 ; i ++){
			Set<String> s = DM.memory.keySet();
			for(String ss : s){
			System.out.println(DM.memory.get(ss));
			}
		}
	}
